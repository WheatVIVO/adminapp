...
public class ABoxRecomputer {
	...
	Set<String> processedIndividuals = new HashSet<String>();
	int previousSize = size;
        while (!individuals.isEmpty()) {
	    int addedWorkload = individuals.size() - (previousSize - 1);
	    if(addedWorkload > 0) {
	        size += addedWorkload;
	    }
	    previousSize = individuals.size();
            String individualURI = individuals.poll();
	    if(processedIndividuals.contains(individualURI)) {
                continue;
	    } else {
                processedIndividuals.add(individualURI);
	    }
            try {
