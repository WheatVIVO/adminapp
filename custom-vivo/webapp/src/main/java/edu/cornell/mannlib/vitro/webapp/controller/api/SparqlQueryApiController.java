...
/**
 * WheatVIVO adminapp change: comment out authorization check,
 * which is insanely too unbelievably, excruciatingly slow
 * for actually using this API for anything
 *
...
//			sendShortResponse(SC_FORBIDDEN, e.getMessage(), resp);
		...
		} catch (AcceptHeaderParsingException e) {
			sendShortResponse(SC_NOT_ACCEPTABLE,
					"The accept header does not include any "
							+ "available content type: " + e.getMessage(), resp);
