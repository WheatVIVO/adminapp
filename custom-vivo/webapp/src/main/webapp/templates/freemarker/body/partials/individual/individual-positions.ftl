...
<#if positions?has_content> <#-- true when the property is in the list, even if not populated (when editing) -->
    ...
    <h2 id="${localName}" class="mainPropGroup f3 pa3 pl0" title="${positions.publicDescription!}">${positions.name?capitalize} <@p.addLink positions editable /> <@p.verboseDisplay positions /></h2>
